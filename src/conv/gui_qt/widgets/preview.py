"""Preview-панель — миниатюра, видеоплеер, медиа-инфо, навигация, таймлайн."""
...
class _ScaledLabel(QLabel):
    """QLabel, масштабирующий QPixmap под свой размер."""
...
class _VideoPlayerWidget(QWidget):
    """Виджет для воспроизведения видео через QMediaPlayer."""
...
def _fmt_time2(sec: float) -> str:
    """MM:SS без дробной части."""
    m, s = divmod(int(sec), 60)
    return f"{m:02d}:{s:02d}"


class PreviewPanel(QWidget):
    """Панель предпросмотра: миниатюра/видео + медиа-инфо + навигация + таймлайн."""
    ...
    def _build_ui(self):
        ...
        # ── Медиа-область (QStackedWidget: 0=изо, 1=видео) ──
        self._media_stack = QStackedWidget()

        # Страница 0: изображение / placeholder
        scroll = QScrollArea()
        ...
        self._media_stack.addWidget(scroll)  # index 0

        # Страница 1: видеоплеер
        self._video_player = _VideoPlayerWidget()
        self._media_stack.addWidget(self._video_player)  # index 1
    ...
    def get_trim(self, path: Path) -> tuple[float, float]:
        """Вернуть trim (start, end) в секундах. (0, 0) = без обрезки."""
    ...
    def show(self, path: Path, idx: int, total: int,
        ...
        if is_media:
            ...
            if info.has_audio:
                ...
                if info.audio_channels:
                    parts.append(ch_map.get(str(info.audio_channels),
        ...
        # ── Медиа-отображение ──
        # Видео → видеоплеер (stack index 1)
        if ext in VIDEO_INPUT:
        ...
        # Аудио → placeholder + таймлайн (stack index 0)
        if ext in AUDIO_INPUT:
            self._media_stack.setCurrentIndex(0)
            self._video_player.unload()
            self._image_label.clear_image()
            self._image_label.setText("🎵\n(аудио — waveform на таймлайне ниже)")
            self._timeline.set_file(path)
            return

        # Изображение → PIL preview (stack index 0)
        self._media_stack.setCurrentIndex(0)
        self._video_player.unload()
        self._timeline.setVisible(False)
        try:
